...
{
	public sealed class CodeEmitterLabel
	{
		internal readonly Label Label;
		internal int Temp;

		public CodeEmitterLabel(Label label)
		{
			this.Label = label;
		}
	}

	public sealed class CodeEmitterLocal
	{
		private Type type;
		private string name;
		private LocalBuilder local;

		public CodeEmitterLocal(Type type)
		{
			this.type = type;
		}
		...
		public int __LocalIndex
		{
			get { return local == null ? 0xFFFF : local.LocalIndex; }
		}

		public void Emit(ILGenerator ilgen, OpCode opcode)
		{
			if (local == null)
			{
		...
		public void Declare(ILGenerator ilgen)
		{
			local = ilgen.DeclareLocal(type);
			if (name != null)
			{
	...
	public sealed class CodeEmitter
	{
		...
		public enum CodeTypeFlags : short
		{
			None = 0,
			EndFaultOrFinally = 1,
		}

		public struct OpCodeWrapper
		{
			public readonly CodeType pseudo;
			private readonly CodeTypeFlags flags;
			public readonly OpCode opcode;
			private readonly object data;

			public OpCodeWrapper(CodeType pseudo, object data)
			{
				this.pseudo = pseudo;
			...
			public OpCodeWrapper(CodeType pseudo, CodeTypeFlags flags)
			{
				this.pseudo = pseudo;
				this.flags = flags;
				this.opcode = OpCodes.Nop;
			...
			public OpCodeWrapper(OpCode opcode, object data)
			{
			...
			public bool Match(OpCodeWrapper other)
			{
			...
			public bool HasLabel => data is CodeEmitterLabel;

			public CodeEmitterLabel Label => (CodeEmitterLabel)data;

			public bool MatchLabel(OpCodeWrapper other)
			{
				return data == other.data;
			}

			public CodeEmitterLabel[] Labels => (CodeEmitterLabel[])data;

			public bool HasLocal => data is CodeEmitterLocal;

			public CodeEmitterLocal Local => (CodeEmitterLocal)data;

			public bool MatchLocal(OpCodeWrapper other)
			{
				return data == other.data;
			}

			public bool HasValueByte => data is byte;

			public byte ValueByte => (byte)data;

			public short ValueInt16 => (short)data;

			public int ValueInt32 => (int)data;

			public long ValueInt64 => (long)data;

			public Type Type => (Type)data;

			public FieldInfo FieldInfo => (FieldInfo)data;

			public MethodBase MethodBase => (MethodBase)data;
			...
			public void RealEmit(int ilOffset, CodeEmitter codeEmitter, ref int lineNumber)
			{
		...
		public sealed class CalliWrapper
		{
			public readonly CallingConvention unmanagedCallConv;
			public readonly Type returnType;
			public readonly Type[] parameterTypes;

			public CalliWrapper(CallingConvention unmanagedCallConv, Type returnType, Type[] parameterTypes)
			{
		...
		public static CodeEmitter Create(MethodBuilder mb)
		{
		...
		public void DefineSymbolDocument(ModuleBuilder module, string url, Guid language, Guid languageVendor, Guid documentType)
		{
		...
		public void ReleaseTempLocal(CodeEmitterLocal lb)
		{
		...
		public void Emit(OpCode opcode, MethodBase mb)
		{
			EmitOpCode(opcode, mb);
		}

		public void EmitLdc_R8(double arg)
		{
		...
		public void EmitLdarga(int arg)
		{
			Debug.Assert(0 <= arg && arg < 65536);
			if (arg < 256)
			{
		...
		public void EmitLdc_I8(long arg)
		{
			EmitOpCode(OpCodes.Ldc_I8, arg);
		}

		public void EmitBr(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Br, label);
		}

		public void EmitBeq(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Beq, label);
		}

		public void EmitBne_Un(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Bne_Un, label);
		}

		public void EmitBle_Un(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Ble_Un, label);
		}

		public void EmitBlt_Un(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Blt_Un, label);
		}

		public void EmitBge_Un(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Bge_Un, label);
		}

		public void EmitBle(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Ble, label);
		}

		public void EmitBlt(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Blt, label);
		}

		public void EmitBge(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Bge, label);
		}

		public void EmitBgt(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Bgt, label);
		}

		public void EmitBrtrue(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Brtrue, label);
		}

		public void EmitBrfalse(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Brfalse, label);
		}

		public void EmitLeave(CodeEmitterLabel label)
		{
			EmitOpCode(OpCodes.Leave, label);
		}

		public void EmitSwitch(CodeEmitterLabel[] labels)
		{
			EmitOpCode(OpCodes.Switch, labels);
		}

		public void Emit(OpCode opcode, CodeEmitterLocal local)
		{
			EmitOpCode(opcode, local);
		}

		public void EmitLdc_R4(float arg)
		{
		...
		public void MarkLabel(CodeEmitterLabel loc)
		{
#if LABELCHECK
			labels.Remove(loc);
#endif
		...
		public void SetLineNumber(ushort line)
		{
			if (symbols != null)
			{
...
#if STATIC_COMPILER
		public void EmitLineNumberTable(MethodBuilder mb)
		{
			if(linenums != null)
			{
		...
		public void EmitThrow(string dottedClassName)
		{
			var exception = ClassLoaderWrapper.GetBootstrapClassLoader().LoadClassByDottedName(dottedClassName);
			var methodWrapper = exception.GetMethodWrapper("<init>", "()V", false);
			methodWrapper.Link();
			methodWrapper.EmitNewobj(this);
			Emit(OpCodes.Throw);
		}

		public void EmitThrow(string dottedClassName, string message)
		{
			var exception = ClassLoaderWrapper.GetBootstrapClassLoader().LoadClassByDottedName(dottedClassName);
			Emit(OpCodes.Ldstr, message);
			var methodWrapper = exception.GetMethodWrapper("<init>", "(Ljava.lang.String;)V", false);
			methodWrapper.Link();
			methodWrapper.EmitNewobj(this);
			Emit(OpCodes.Throw);
		}
		...
		public void EmitAssertType(Type type)
		{
		...
		public void EmitUnboxSpecial(Type type)
		{
		...
		public void Emit_instanceof(Type type)
		{
			Emit(OpCodes.Isinst, type);
			Emit(OpCodes.Ldnull);
			Emit(OpCodes.Cgt_Un);
		...
		public void Emit_if_le_lt_ge_gt(Comparison comp, CodeEmitterLabel label)
		{
