...
namespace IKVM.Internal
{
	class AssemblyClassLoader : ClassLoaderWrapper
	{
		...
		private AssemblyClassLoader[] _delegates;
		...
		private sealed class AssemblyLoader
		{
			...
			private readonly bool[] _isJavaModule;
			private readonly Module[] _modules;
			private readonly Dictionary<string, string> _nameMap;
			private readonly bool _hasDotNetModule;
			private AssemblyName[] _internalsVisibleTo;
			private string[] _jarList;
#if !STATIC_COMPILER && !STUB_GENERATOR && !FIRST_PASS
			...
			internal AssemblyLoader(Assembly assembly)
			{
				...
				_isJavaModule = new bool[_modules.Length];
				for (var i = 0; i < _modules.Length; i++)
				{
					var moduleAttributes = AttributeHelper.GetJavaModuleAttributes(_modules[i]);
					if (moduleAttributes.Length > 0)
					{
						_isJavaModule[i] = true;
						foreach (JavaModuleAttribute jma in moduleAttributes)
						{
							...
							if (jars != null)
							{
								...
								{
									string[] newList = new string[_jarList.Length + jars.Length];
									Array.Copy(_jarList, newList, _jarList.Length);
									Array.Copy(jars, 0, newList, _jarList.Length, jars.Length);
									_jarList = newList;
								}
							}
						}
			...
			internal bool HasJavaModule
			{
				get
				{
					for (int i = 0; i < _isJavaModule.Length; i++)
					{
						if (_isJavaModule[i])
						{
							return true;
			...
			private Type GetJavaType(Module mod, string name)
			{
				try
				{
					...
					if (_nameMap != null)
					{
						_nameMap.TryGetValue(name, out n);
					}
			...
			internal TypeWrapper DoLoad(string name)
			{
				for (int i = 0; i < _modules.Length; i++)
				{
					if (_isJavaModule[i])
					{
						Type type = GetJavaType(_modules[i], name);
						if (type != null)
						{
					...
					else
					{
						...
						Type type = GetType(_modules[i], DotNetTypeWrapper.DemangleTypeName(name));
			...
			internal bool InternalsVisibleTo(AssemblyName otherName)
			{
				...
				{
					Interlocked.CompareExchange(ref _internalsVisibleTo, AttributeHelper.GetInternalsVisibleToAttributes(_assembly), null);
				}
				foreach (AssemblyName name in _internalsVisibleTo)
				{
			...
			internal java.util.Enumeration FindResources(string name)
			{
				if (urlClassPath == null)
				{
					...
					{
						urls.Add(MakeResourceURL(_assembly, jar));
					}
			...
			: base(CodeGenOptions.None, null)
		{
			...
			this._references = fixedReferences;
		}
		...
		private void DoInitializeExports()
		{
			lock (this)
			{
				if (_delegates == null)
				{
					if (!(ReflectUtil.IsDynamicAssembly(_assemblyLoader.Assembly)) && _assemblyLoader.Assembly.GetManifestResourceInfo("ikvm.exports") != null)
					{
						List<string> wildcardExports = new List<string>();
						using (Stream stream = _assemblyLoader.Assembly.GetManifestResourceStream("ikvm.exports"))
						{
							BinaryReader rdr = new BinaryReader(stream);
							int assemblyCount = rdr.ReadInt32();
							...
							for (int i = 0; i < assemblyCount; i++)
							{
								...
								int typeCount = rdr.ReadInt32();
								if (typeCount == 0 && _references == null)
								{
						...
						if (_references == null)
						{
							_references = wildcardExports.ToArray();
						}
					}
					else
					{
						AssemblyName[] refNames = _assemblyLoader.Assembly.GetReferencedAssemblies();
						_references = new string[refNames.Length];
						for (int i = 0; i < _references.Length; i++)
						{
							_references[i] = refNames[i].FullName;
						}
					}
					Interlocked.Exchange(ref _delegates, new AssemblyClassLoader[_references.Length]);
				}
			}
		}
		...
		internal Assembly MainAssembly
		{
			get
			{
				return _assemblyLoader.Assembly;
			}
		}
		...
		{
			if (_assemblyLoader.Assembly == assembly)
			{
		...
		{
			if (!_assemblyLoader.HasJavaModule)
			{
		...
		private TypeWrapper FindReferenced(string name)
		{
			for (int i = 0; i < _delegates.Length; i++)
			{
				...
				{
					Assembly asm = LoadAssemblyOrClearName(ref _references[i], false);
					if (asm != null)
					{
						_delegates[i] = AssemblyClassLoader.FromAssembly(asm);
					}
				}
				if (_delegates[i] != null)
				{
					TypeWrapper tw = _delegates[i].DoLoad(name);
					if (tw != null)
					{
						return RegisterInitiatingLoader(tw);
					}
		...
		internal void AddDelegate(AssemblyClassLoader acl)
		{
			...
			lock (this)
			{
				_delegates = ArrayUtil.Concat(_delegates, acl);
			}
		}
